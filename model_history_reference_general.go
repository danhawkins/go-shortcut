/*
Shortcut API

Shortcut API

API version: 3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the HistoryReferenceGeneral type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &HistoryReferenceGeneral{}

// HistoryReferenceGeneral A default reference for entity types that don't have extra fields.
type HistoryReferenceGeneral struct {
	// The ID of the entity referenced.
	Id map[string]interface{} `json:"id"`
	// The type of entity referenced.
	EntityType string `json:"entity_type"`
	// The name of the entity referenced.
	Name string `json:"name"`
}

// NewHistoryReferenceGeneral instantiates a new HistoryReferenceGeneral object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewHistoryReferenceGeneral(id map[string]interface{}, entityType string, name string) *HistoryReferenceGeneral {
	this := HistoryReferenceGeneral{}
	this.Id = id
	this.EntityType = entityType
	this.Name = name
	return &this
}

// NewHistoryReferenceGeneralWithDefaults instantiates a new HistoryReferenceGeneral object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewHistoryReferenceGeneralWithDefaults() *HistoryReferenceGeneral {
	this := HistoryReferenceGeneral{}
	return &this
}

// GetId returns the Id field value
func (o *HistoryReferenceGeneral) GetId() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *HistoryReferenceGeneral) GetIdOk() (map[string]interface{}, bool) {
	if o == nil {
		return map[string]interface{}{}, false
	}
	return o.Id, true
}

// SetId sets field value
func (o *HistoryReferenceGeneral) SetId(v map[string]interface{}) {
	o.Id = v
}

// GetEntityType returns the EntityType field value
func (o *HistoryReferenceGeneral) GetEntityType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.EntityType
}

// GetEntityTypeOk returns a tuple with the EntityType field value
// and a boolean to check if the value has been set.
func (o *HistoryReferenceGeneral) GetEntityTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.EntityType, true
}

// SetEntityType sets field value
func (o *HistoryReferenceGeneral) SetEntityType(v string) {
	o.EntityType = v
}

// GetName returns the Name field value
func (o *HistoryReferenceGeneral) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *HistoryReferenceGeneral) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *HistoryReferenceGeneral) SetName(v string) {
	o.Name = v
}

func (o HistoryReferenceGeneral) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o HistoryReferenceGeneral) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["entity_type"] = o.EntityType
	toSerialize["name"] = o.Name
	return toSerialize, nil
}

type NullableHistoryReferenceGeneral struct {
	value *HistoryReferenceGeneral
	isSet bool
}

func (v NullableHistoryReferenceGeneral) Get() *HistoryReferenceGeneral {
	return v.value
}

func (v *NullableHistoryReferenceGeneral) Set(val *HistoryReferenceGeneral) {
	v.value = val
	v.isSet = true
}

func (v NullableHistoryReferenceGeneral) IsSet() bool {
	return v.isSet
}

func (v *NullableHistoryReferenceGeneral) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableHistoryReferenceGeneral(val *HistoryReferenceGeneral) *NullableHistoryReferenceGeneral {
	return &NullableHistoryReferenceGeneral{value: val, isSet: true}
}

func (v NullableHistoryReferenceGeneral) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableHistoryReferenceGeneral) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


